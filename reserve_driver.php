...
#$reserve = new Reserve();
#$reserve->attemptBooking($slug, $datetime, $minutes_before, $minutes_after, $party_size, $email_address, $phone_number, $first_name, $last_name);
...
Class Reserve {
	...
	function attemptBooking($slug, $datetime, $minutes_before, $minutes_after, $party_size, $email_address, $phone_number, $first_name, $last_name) {
		...
		$info = $this->getRestaurantInfo($slug, $datetime, $time_radius, $party_size);
		print_r($info);
		...
		$slot = $this->getSlot($available_slots, $datetime, $minutes_before, $minutes_after);
		print_r($slot);

		if ($slot) {
			print "Found a Time Slot\n";
			$reservation = $this->getReservationInfo($slug, $datetime, $party_size, $slot, $venue_id);
			$reservation_id = $reservation->data->id;
			$venue_id = $reservation->data->venue_id;
			if ($reservation_id) {
				print "Reservation ID: $reservation_id \n";
				$submit_result = $this->submitReservation($account_id, $venue_id, $email_address, $phone_number, $first_name, $last_name);
				$guest_id = $submit_result->data->id;
				$result1 = $this->putReservation1($reservation_id, $guest_id);
				$result2 = $this->putReservation2($reservation_id, $guest_id);
